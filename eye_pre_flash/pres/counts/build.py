...
(`eye_pre_flash.corr.labels.SOURCE_SCOPES["svm"]`), applied to *every* label
...
Usage:
    uv run python -m eye_pre_flash.pres.counts.build
    uv run python -m eye_pre_flash.pres.counts.build --source dendro --monkey Nielsen
    uv run python -m eye_pre_flash.pres.counts.build --scope allplus
...
from eye_pre_flash.corr.labels import SOURCES, source_scope_sessions
