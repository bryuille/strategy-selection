...
`eye_pre_flash.maze_strategy_pairs`, restricted to the `mean_removed` variant and
...
the package's own `build.py`; this is
the slide version.
...
`maze_strategy_pairs` output gives this variant a zero-centred diverging map
...
`eye_pre_flash/maze_strategy_pairs/build.py`. These slides use parula anyway for a
