"""Per-trial feature blocks with the codebook fitted per (session, maze).

The same pipeline as `classifier.features`, with step 3 moved: instead of one
k-means per (monkey, K) pooled over every session and every maze, this fits
**one k-means per (session, maze, K)**. Every other step -- the window clip,
the unit-H warp, I-DT fixation detection, the nearest-prototype assignment --
is unchanged and reuses `classifier.features`' own helpers, so the only
difference between a row here and the corresponding row there is which
prototypes it was assigned against.

`classifier.features` is left completely alone. The K = 6 / K = 12 pooled
codebooks every other analysis in the repo reads are the same bits they were;
this module writes to its own cache stem and never touches theirs.

Consequences of the narrower fit, which callers have to respect:

* A dimension means nothing across groups. State 3 of `june_24_g0` maze 2 is
  not state 3 of `june_25_g0` maze 2 -- different k-means, different centres.
  So features may only ever be compared *within* one (session, maze) group,
  which is exactly what `comp_build` does.
* `no_origin`'s origin state and `mean_removed`'s grand mean are therefore
  per-group too, and must be recomputed from that group's own codebook and
  its own trials rather than inherited from a monkey-wide fit.
* A group with fewer pooled samples than K cannot be fitted at all. Those are
  dropped, and their `(session, maze)` recorded in `dropped_groups`.

Caches land under ``data/processed/<Monkey>_mspcomp_k<K>_unith_s<start>_e<end>.npz``.
"""
...
# would dominate the cache at K = 30 for a block no caller here uses.
BLOCK_NAMES = ("occ_ms", "occ_bin")


def _fit_group_codebook(pool_xy, k, *, seed=SEED):
    """K-means over one (session, maze)'s pooled in-window fixation samples.

    Identical to `classifier.features._fit_window_codebook` except for what it
    is handed. Returns ``(centers, n_pool)``, or ``(None, n_pool)`` when the
    group has fewer samples than K and no k-means is possible.
    """
...
def extract_monkey_features(
    ...
    """Feature blocks for every usable trial of `monkey`, per-group codebooks.
    ...
    `group_session` / `group_maze` / `codebook_xy` arrays, so a caller can
    recover the codebook any given row was assigned against.
    """
    ...
    # ---- Steps 1-2: keep in-window samples, pooled **per (session, maze)** --
    rng = np.random.default_rng(SEED)
    usable = []
    pool = {}
    for i in range(sessions.size):
        ...
        pool.setdefault((session, maze), []).append(
            np.stack([x[idx], y[idx]], axis=1)
        )
        usable.append((i, session, trial_id, maze, lo, hi))

    radius = ASSIGN_RADIUS

    # ---- Step 3: one k-means per (session, maze) ---------------------------
    codebooks, group_session, group_maze, dropped = {}, [], [], []
    for key in sorted(pool):
        centers, n_pool = _fit_group_codebook(pool[key], k)
        if centers is None:
            dropped.append(f"{key[0]}/maze{key[1]}: {n_pool} samples < K={k}")
            continue
        codebooks[key] = (len(group_session), centers)
        group_session.append(key[0])
        group_maze.append(key[1])

    if dropped:
        print(f"  K={k}: {len(dropped)} group(s) unfittable: {'; '.join(dropped)}")
    print(f"  K={k}: {len(codebooks)} (session, maze) codebooks fitted")

    blocks = {name: [] for name in BLOCK_NAMES}
    rows_session, rows_trial, rows_maze, rows_group = [], [], [], []

    # ---- Steps 4-5: assign each trial against *its own group's* codebook ---
    for i, session, trial_id, maze, lo, hi in usable:
        entry = codebooks.get((session, maze))
        if entry is None:
            continue
    ...
    out["group_session"] = np.asarray(group_session, dtype=str)
    out["group_maze"] = np.asarray(group_maze, dtype=int)
    out["codebook_xy"] = (
        np.stack([codebooks[key][1] for key in sorted(codebooks)], axis=0)
        if codebooks
...
def cache_stem(monkey, k, start_ms=DEFAULT_START_MS, end_ms=DEFAULT_END_MS):
    """Distinct from `classifier.features.cache_stem` on purpose.

    `mspcomp` rather than `clf2`, so a per-group codebook can never be served
    to a caller that asked for the pooled one, at any K the two share.
    """
    return f"{monkey}_mspcomp_k{int(k)}_unith_s{int(start_ms)}_e{int(end_ms)}"


def load_features(
    monkey, *, k, start_ms=DEFAULT_START_MS, end_ms=DEFAULT_END_MS, refresh=False
):
    """Cached per-(session, maze) feature blocks for `monkey` at `k`."""
    path = processed_npz(cache_stem(monkey, k, start_ms, end_ms))
    if path.exists() and not refresh:
        return load_npz(path)
    print(f"Extracting {monkey} per-(session, maze) features (k={k}) ...")
