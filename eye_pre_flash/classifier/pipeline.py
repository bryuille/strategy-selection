"""One command from `data/mat/` to every eye_pre_flash/classifier table.
...
Usage:
    ...
    uv run python -m eye_pre_flash.classifier.pipeline --from decode-pub
...
def stages():
    ...
    return {
        ...
        "features": ["eye_pre_flash.classifier.features"],
        "decode-pub": ["eye_pre_flash.classifier.decoding", "--scope", "publication"],
        "decode-all": ["eye_pre_flash.classifier.decoding", "--scope", "all"],
...
def run(name, argv, *, dry_run):
    ...
    if result.returncode != 0:
        raise SystemExit(
            ...
            f"python -m eye_pre_flash.classifier.pipeline --from {name}"
        )
