"""Cross-validated unit-H heatmap similarities on a linear count/trial scale.

Same split-half CV as ``similarities_heatmap_log``, but each
half map is ``count / trial`` rather than ``log1p(count / trial)``.

Usage:
    uv run python -m eye_pre_flash.plotting.similarities_heatmap --monkey Faure
    uv run python -m eye_pre_flash.plotting.similarities_heatmap --monkey Nielsen
    uv run python -m eye_pre_flash.plotting.similarities_heatmap --monkey Faure --bin-w 0.142857
"""
...
VISUALIZER = "similarities_heatmap"
