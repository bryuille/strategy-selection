"""Shared session-averaged split-half CV similarity grids."""
...
BLUE_YELLOW = LinearSegmentedColormap.from_list("parula", _PARULA_RGB, N=256)
...
def split_half_indices(n, rng):
    order = rng.permutation(n)
    mid = n // 2
    return order[:mid], order[mid:]


def session_cv_matrix(features, mazes, rng, n_splits=N_SPLITS):
    ...
    for _ in range(n_splits):
        ...
        for i, rows in enumerate(by_maze):
            if len(rows) < 2:
                continue
            idx_a, idx_b = split_half_indices(len(rows), rng)
        ...
        for i in range(6):
            for j in range(6):
                ...
                if mean_b[i] is None or mean_a[j] is None:
                    continue
                r = np.nanmean(
                    [pearson(mean_a[i], mean_b[j]), pearson(mean_b[i], mean_a[j])]
                )
                if np.isfinite(r):
                    acc[i, j] += r
                    counts[i, j] += 1
...
def session_averaged_cv(features, sessions, mazes, *, n_splits=N_SPLITS, seed=0):
    """Return (6×6 mean CV *r*, per-maze trial counts, n sessions)."""
    rng = np.random.default_rng(seed)
    session_mats = []
    for session in np.unique(sessions):
        mask = sessions == session
        mat = session_cv_matrix(
